...
const App = () => {
    // Determine the current state for selection defaults
    const currentYear = new Date().getFullYear();
    const [year, setYear] = useState(null); // Initial year is null to trigger scoreboard
    const [week, setWeek] = useState(null); 
    
    // State to trigger fetching only when the button is clicked or on initial load
    ...
    // Define targetYear in App scope so it's accessible in render and fetch functions
    ...
    // Dynamically calculate the requested API URL based on state
    ...
    const extractTeams = useCallback((events) => {
        ...
        events.forEach(event => {
            ...
            if (oddsList && oddsList.length > 0) {
                ...
                // It's either nested in 'items[0]', or the primary object itself.
    ...
    /**
     * Core fetching logic. Now uses the deep fetch for all cases.
     */
    ...
    return (
        <div className="p-4 sm:p-8 max-w-3xl mx-auto bg-gray-50 min-h-screen">
            ...
            {/* Debug Toggle - MOVED TO THE BOTTOM AND STYLED TO BE LESS VISIBLE */}
